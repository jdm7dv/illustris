;  $Id: //depot/idl/IDL_70/idldir/examples/doc/widgets/tab_widget_example2.pro#2 $
...
PRO tab_widget_example2_event, ev
  ...
  ; cycle through the tabs by calling the 'TWE2_SwitchTab' procedure.
  ...
  ; the various widgets, as contained in the 'retStruct' structure.
  ; In a real application, this step would probably adjust settings
  ; in the application to reflect the changes made by the user.
...
PRO tab_widget_example2, LOCATION=location
  ...
  ; that will hold the widget's value. Specify the 'TWE2_saveValue'
  ...
  wSlider = WIDGET_SLIDER(wT2, UVALUE=2, EVENT_FUNC='TWE2_saveValue')
