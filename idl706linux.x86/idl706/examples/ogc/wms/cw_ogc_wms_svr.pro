...
;;----------------------------------------------------------------------------
pro cw_ogc_wms_svr_reload_servers, ev, lastSvr
  compile_opt idl2
...
;    svr = widget_info((*pstate).wcbSvrList, /combobox_gettext)
  ...
  if (size(*(*pstate).paxServers, /type) ne 8) then begin
     ...
     widget_control, (*pstate).wtxtWmsVer,         set_value = '1.1.1'
     return
  endif
  ...
  widget_control, (*pstate).wtxtWmsVer,         set_value = (*(*pstate).paxServers)[svrMatch].WmsVer
end

;;----------------------------------------------------------------------------
pro cw_ogc_wms_svr_cbSvrs_event, ev
  compile_opt idl2
  ...
  for xx = 0, count-1 do begin
     if (svrName eq (*(*pstate).paxServers)[xx].SvrName) then begin
         ...
         widget_control, (*pstate).wtxtWmsVer,         set_value = (*(*pstate).paxServers)[xx].WmsVer
         break;
     endif
  endfor
...
;;----------------------------------------------------------------------------
pro cw_ogc_wms_svr_btnSave_event, ev
  compile_opt idl2
  ...
  cw_ogc_wms_svr_Save, ev, 0
end

;;----------------------------------------------------------------------------
pro cw_ogc_wms_svr_btnNew_event, ev
  compile_opt idl2
...
;;----------------------------------------------------------------------------
pro cw_ogc_wms_svr_btnDelete_event, ev
  compile_opt idl2
  ...
  cw_ogc_wms_svr_reload_servers, ev, 0
  cw_ogc_wms_svr_Save, ev, 1
end

;;----------------------------------------------------------------------------
pro cw_ogc_wms_svr_btnParse_event, ev
  compile_opt idl2
  ...
  if (val eq '') then begin
    r = dialog_message('Paste in OGC Server URL before pressing Parse.', title='OGC WMS Error', dialog_parent=ev.top, /error)
    return
  endif

;  valUp = STRUPCASE(val)
;  if (strpos(valUp, 'SERVICE=') eq -1) then begin
;    r = dialog_message('Invalid OGC URL...it does not contain a Service= segment.', title='OGC WMS Error', dialog_parent=ev.top, /error)
;    return
;  endif

  ; create a ogc wms object..it is needed to convert url's into properties
  owms = obj_new('IDLnetOgcWms')
  owms->ParseURL, val[0]
  owms->getproperty, url_scheme=scheme, url_host=host, url_port=port, url_path=path, url_query_prefix=queryprefix, url_query_suffix=querysuffix, wms_version=wmsver
  obj_destroy, owms
  ...
  widget_control, (*pstate).wtxtWmsVer,     set_value = wmsver
end


;;----------------------------------------------------------------------------
function cw_ogc_wms_svr_CreateSvrStruct
  compile_opt idl2
...
;;----------------------------------------------------------------------------
pro cw_ogc_wms_svr_Save, ev, returnOnBlank
  compile_opt idl2
  ...
  xSvr = cw_ogc_wms_svr_CreateSvrStruct()
  widget_control, (*pstate).wtxtSvrName,    get_value = val
  xSvr.SvrName = val
  if (val eq '') then begin
    if (returnOnBlank gt 0) then return
    r = dialog_message('Entry Name can not be null', title='OGC WMS Error', dialog_parent=ev.top, /error)
    return
  endif
  ...
  if (val eq '') then begin
    if (returnOnBlank gt 0) then return
    r = dialog_message('Server Name can not be null', title='OGC WMS Error', dialog_parent=ev.top, /error)
    return
  endif
  ...
  if (val eq '') then begin
    if (returnOnBlank gt 0) then return
    r = dialog_message('Host Name can not be null', title='OGC WMS Error', dialog_parent=ev.top, /error)
    return
  endif
  ...
  if (val eq '') then begin
    if (returnOnBlank gt 0) then return
    r = dialog_message('Port can not be null', title='OGC WMS Error', dialog_parent=ev.top, /error)
    return
  endif
  ...
  if (val eq '') then begin
    if (returnOnBlank gt 0) then return
    r = dialog_message('Path can not be null', title='OGC WMS Error', dialog_parent=ev.top, /error)
    return
  endif
  ...
  widget_control, (*pstate).wtxtWmsVer,get_value = val
  xSvr.WmsVer = val
  if (val eq '') then begin
    if (returnOnBlank gt 0) then return
    r = dialog_message('Version can not be null', title='OGC WMS Error', dialog_parent=ev.top, /error)
    return
  endif

  if (alreadyExists ne -1) then begin
    (*(*pstate).paxServers)[xx].WmsVer = val
  endif
  ...
  cw_ogc_wms_svr_reload_servers, ev, 1
  ogc_wms_SaveValuesAndNotify, ev.top, 'svrstatebase'
end

;;----------------------------------------------------------------------------
pro cw_ogc_wms_svr_kill_event, id
  compile_opt idl2
  catch, errorStatus            ; catch all errors and display an error dialog
...
pro cw_ogc_wms_svr_refresh, id
  ...
  prefsFile = ogc_wms_getConfigInfo(id, 'svrstatebase')

end

;;----------------------------------------------------------------------------
pro cw_ogc_wms_svr_realize_notify_event, id
  compile_opt idl2
  ...
  widget_control, (*pstate).wtxtWmsVer,         set_value = (*(*pstate).paxServers)[0].WmsVer

end

;;----------------------------------------------------------------------------
function cw_ogc_wms_svr, parent  ;, CALLBACKROUTINE=callbackRoutine
  compile_opt idl2
  on_error, 2
...
;  endif
  ...
  wBase             = widget_base(parent, /COLUMN, PRO_SET_VALUE='cw_ogc_wms_svr_set_value', $
                                    NOTIFY_REALIZE='cw_ogc_wms_svr_realize_notify_event', space=5)

  wBaseState        = widget_base(wBase, uname='svrstatebase', kill_notify='cw_ogc_wms_svr_kill_event')
  ...
  wcbSvrList        = widget_combobox(wSvrr, scr_xsize=lxsize*boxsize, event_pro='cw_ogc_wms_svr_cbSvrs_event')
  ...
  wlbl              = widget_label(wPathr, value='  Path to a CGI routine that handles OGC requests.')
  ...
  wlbl              = widget_label(wQueryPrefixr, value='  Optional: Prefix to Service=WMS&Request=Get...')
  ...
  wlbl              = widget_label(wQuerySuffixr, value='  Optional: Suffix to Service=WMS&Request=Get...')

  wWmsVerr          = widget_base(wbaseFrSvr, /row)
  wlblWmsVer        = widget_label(wWmsVerr, value='WMS Version Supported ', /align_right, xsize=lxsize*textsize)
  wtxtWmsVer        = widget_text(wWmsVerr, /edit, value = '1.1.1', scr_xsize=lxsize*boxsize)
  wlbl              = widget_label(wWmsVerr, value='  WMS Version supported by both the client and server.')

  wBaseCol2         = widget_base(wbaseFrSvr, /Col, space=10, xpad=5)
  wSvrr             = widget_base(wBaseCol2, /row, xpad=140)
  wbtnSvrNew        = widget_button(wSvrr, value='New', xsize = 80, uvalue=0, event_pro='cw_ogc_wms_svr_btnNew_event')
  wbtnSvrDelete     = widget_button(wSvrr, value='Delete', xsize = 80, event_pro='cw_ogc_wms_svr_btnDelete_event')
  wbtnSvrSave       = widget_button(wSvrr, value='Save Changes', xsize = 100, event_pro='cw_ogc_wms_svr_btnSave_event')
  ...
  wUrlParser        = widget_base(wbaseFrSvr, /row)
  wtxtUrlParse      = widget_text(wUrlParser, /editable, scr_xsize=640)
  wbtnUrlParse      = widget_button(wUrlParser, value='Parse', xsize = 80, event_pro='cw_ogc_wms_svr_btnParse_event')
  ...
  state = { wcbSvrList:wcbSvrList, wtxtSvrName:wtxtSvrName, wtxtScheme:wtxtScheme, wtxtHost:wtxtHost, $
            ...
            wtxtUrlParse:wtxtUrlParse, wtxtWmsVer:wtxtWmsVer,  $
  ...
  (*pstate).prefsFile = ogc_wms_getConfigInfo(wBase, 'svrstatebase')
