; $Id: //depot/idl/IDL_70/idldir/lib/itools/components/idlitvisplot3d__define.pro#2 $
;
...
PRO IDLitVisPlot3d::OnDataDisconnect, ParmName
   ...
   case ParmName of
       'X': begin
           ...
           data=0b
           self._oPolyline->SetProperty, datax=indgen(szDims[1])
       end
       'Y': begin
           self._oPolyline->GetProperty, data=data
           szDims = size(data,/dimensions)
           data=0b
           self._oPolyline->SetProperty, datay=indgen(szDims[1])
       end
       'Z': begin
           self._oPolyline->GetProperty, data=data
           szDims = size(data,/dimensions)
           data=0b
           self._oPolyline->SetProperty, dataz=indgen(szDims[1])
       end
       'X ERROR': begin
