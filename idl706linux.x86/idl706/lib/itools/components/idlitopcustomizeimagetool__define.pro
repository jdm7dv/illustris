; $Id: //depot/idl/IDL_70/idldir/lib/itools/components/idlitopcustomizeimagetool__define.pro#2 $
;
        ...
        if (nTargets gt 0) then begin
            ...
            if (oTarget->_GetImageDimensions(imgDims)) then begin
                ...
                if ((viewportLoc[0] eq 0) && $
                    ...
                    ; Ensure new margins are clamped to two decimal places
                    ; since that is the restriction imposed by both the
                    ; registered margin properties and internal layer
                    ; computations.
                    newMargins = (DOUBLE(newWinDims - imgDims) / $
                        newWinDims) * 0.5
                    newMargins = ((ROUND(newMargins*100.0d)/100.0d) > 0.0) < $
                        maxMargin
                    newWinDims = ROUND(imgDims / (1.0 - (newMargins*2)))
                    ...
                    ; registered margin properties and internal layer
                    ; computations.
                    newMargins = (DOUBLE(newDims - imgDims) / $
                        newDims) * 0.5
                    newMargins = ((ROUND(newMargins*100.0d)/100.0d) > 0.0) < $
                        maxMargin
                    newDims = ROUND(imgDims / (1.0 - (newMargins*2)))
