; $Id: //depot/idl/IDL_70/idldir/lib/itools/components/idlitviscontourlevel__define.pro#2 $
;
...
pro IDLitVisContourLevel::_RegisterProperties
   ...
   compile_opt idl2, hidden
    ...
    ;*** Note: All IDLitVisContourLevel properties should start with MY_
    ;    so we can filter on them within IDLitVisContour::SetProperty.

    self->RegisterProperty, 'MY_VALUE', /FLOAT, $
        NAME='Value', $
        DESCRIPTION='Contour value', $
        /HIDE  ; need to disable for styles, until we have data

    self->RegisterProperty, 'MY_COLOR', /COLOR, $
        NAME='Color', $
        DESCRIPTION='Contour color'

    self->RegisterProperty, 'MY_LINESTYLE', /LINESTYLE, $
        NAME='Line style', $
        DESCRIPTION='Contour linestyle'

    self->RegisterProperty, 'MY_THICK', /THICKNESS, $
        NAME='Line thickness', $
        DESCRIPTION='Contour thickness'

    self->RegisterProperty, 'MY_LABELLING', $
        ENUMLIST=['None', 'Value', 'Text', 'Symbol'], $
        NAME='Label', $
        DESCRIPTION='Contour labelling'

    self->RegisterProperty, 'MY_LABEL_INTERVAL', /FLOAT, $
        NAME='Label interval', $
        DESCRIPTION='Normalized interval between labels', $
        VALID_RANGE=[0.05d,1d,0.05d]

    self->RegisterProperty, 'MY_LABEL_NOGAPS', /BOOLEAN, $
        NAME='No label gaps', $
        DESCRIPTION='Do not have gaps for labels'

    self->RegisterProperty, 'MY_USE_LABEL_COLOR', /BOOLEAN, $
        NAME='Use label color', $
        DESCRIPTION='Use provided label color instead of default'

    self->RegisterProperty, 'MY_LABEL_COLOR', /COLOR, $
        NAME='Label color', $
        DESCRIPTION='Color of labels'

    self->RegisterProperty, 'MY_LABEL_SYMBOL', /SYMBOL, $
        NAME='Symbol label', $
        DESCRIPTION='Contour symbol label'

    self->RegisterProperty, 'MY_SYMBOL_SIZE', /FLOAT, $
        NAME='Symbol size', $
        DESCRIPTION='Contour symbol size', $
        VALID_RANGE=[0,1,0.05d]

    self->RegisterProperty, 'MY_LABEL_TEXT', /STRING, $
        NAME='Text label', $
    ...
    ; These should have started with MY_. Too late.
...
pro IDLitVisContourLevel::GetProperty, $
    ...
    INDEX=index, $
    MY_COLOR=mycolor, $
    MY_LABEL_COLOR=myLabelColor, $
    MY_LABEL_INTERVAL=myLabelInterval, $
    MY_LABEL_NOGAPS=myLabelNogaps, $
    MY_LABEL_SYMBOL=myLabelSymbol, $
    MY_LABEL_TEXT=myLabelText, $
    MY_LABELLING=myLabelling, $
    MY_LINESTYLE=mylinestyle, $
    MY_SYMBOL_SIZE=mySymbolSize, $
    MY_THICK=mythick, $
    MY_USE_LABEL_COLOR=myUseLabelColor, $
    MY_VALUE=myvalue, $
    ...
    if (ARG_PRESENT(myLabelling)) then $
        myLabelling = self._labelling
    ...
    if (ARG_PRESENT(myvalue)) then begin
        ...
        if haveContour then begin
            ...
            if (self._index lt N_ELEMENTS(c_value)) then $
...
pro IDLitVisContourLevel::SetProperty, $
    ...
    if (N_ELEMENTS(oContour)) then begin
        ...
        self->SetPropertyAttribute, 'MY_VALUE', HIDE=0
    endif
    ...
    if (N_ELEMENTS(myLabelling)) then BEGIN
        wasLabelling = self._labelling
        self._labelling = 0 > myLabelling < 3

        ; First turn the C_LABEL_SHOW on or off.
        self._oContour->GetProperty, C_LABEL_SHOW=c_label_show, $
            C_LABEL_OBJECTS=c_label_objects
        self->_InsertProperty, c_label_show, nlevels, (myLabelling < 1)
        self._oContour->SetProperty, C_LABEL_SHOW=c_label_show

        ; Turn on/off labelling properties.
        self->SetPropertyAttribute, $
            ['MY_USE_LABEL_COLOR', $
            'MY_LABEL_INTERVAL', 'MY_LABEL_NOGAPS'], $
            SENSITIVE=(self._labelling gt 0)
        self->SetPropertyAttribute, $
          ['TICK_DEFINEDFORMAT'], $
          SENSITIVE=(self._labelling EQ 1)
        self->GetProperty,TICK_DEFINEDFORMAT=tickdef
        self->SetPropertyAttribute,['TICKFORMAT'], $
          SENSITIVE=((tickdef EQ 1) AND (self._labelling EQ 1))

        ; This is a tricky one. Only turn on label color prop
        ; if USE_LABEL_COLOR is also set.
        if (~wasLabelling && self._labelling) then begin
            self->GetProperty, MY_USE_LABEL_COLOR=myCurrentUseLabelColor
            self->SetPropertyAttribute, $
                'MY_LABEL_COLOR', SENSITIVE=myCurrentUseLabelColor
        endif else if (~self._labelling) then begin
            self->SetPropertyAttribute, $
                'MY_LABEL_COLOR', SENSITIVE=0
        endif

        ; Turn on/off value/text properties.
        self->SetPropertyAttribute, $
            ['FONT_INDEX', 'FONT_STYLE', 'FONT_SIZE'], $
            SENSITIVE=(self._labelling eq 1) || (self._labelling eq 2)

        ; Turn on/off text-only properties.
        self->SetPropertyAttribute, 'MY_LABEL_TEXT', $
            SENSITIVE=(self._labelling eq 2), HIDE=0

        ; Turn on/off symbol-only properties.
        self->SetPropertyAttribute, ['MY_LABEL_SYMBOL', 'MY_SYMBOL_SIZE'], $
            SENSITIVE=(self._labelling eq 3)

        switch self._labelling of
            ...
            2: begin   ; Text
                ...
                if (self._labelling eq 1) then BEGIN
        ...
        if (OBJ_VALID(oTool)) then begin
            oTool->DoOnNotify, $
                self->GetFullIdentifier(),'SETPROPERTY','MY_LABELLING'
        endif
    ...
    if (N_ELEMENTS(myLabelSymbol)) then begin
        ...
        if (OBJ_VALID(self._oSymbol)) then begin
            ...
            recomputeLabels = 1
            self->DoOnNotify, $
              self->GetFullIdentifier(),'SETPROPERTY','MY_LABEL_SYMBOL'
        endif
    endif
    ...
    if (N_ELEMENTS(mySymbolSize)) then begin
        ...
        if (OBJ_VALID(self._oSymbol)) then begin
            ...
            recomputeLabels = 1
            self->DoOnNotify, $
              self->GetFullIdentifier(),'SETPROPERTY','MY_SYMBOL_SIZE'
        endif
    endif
    ...
    if (N_ELEMENTS(myLabelText)) then begin
        ...
        if (OBJ_VALID(self._oText)) then begin
            ...
            self->DoOnNotify, $
              self->GetFullIdentifier(),'SETPROPERTY','MY_LABEL_TEXT'
            recomputeLabels = 1
        endif
    endif
    ...
    if (N_ELEMENTS(myUseLabelColor)) then begin
        ...
        ; Assume if MY_USE_LABEL_COLOR is sensitive, then it is okay
        ; to sensitize MY_LABEL_COLOR also.
        ; Desensitize is okay no matter what.
        self->SetPropertyAttribute, 'MY_LABEL_COLOR', $
            SENSITIVE=myUseLabelColor

    endif

    ; Only set my_value if we are actually a valid level
