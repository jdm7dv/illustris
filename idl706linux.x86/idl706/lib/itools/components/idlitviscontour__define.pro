; $Id: //depot/idl/IDL_70/idldir/lib/itools/components/idlitviscontour__define.pro#2 $
;
...
function IDLitVisContour::Init, NAME=NAME,$
   ...
   ; We assume that we are PLANAR initially.
    ...
    self._oLevels = OBJ_NEW('_IDLitVisualization', $
        NAME='All levels', /PRIVATE, /PROPERTY_INTERSECTION)
    ...
    oLevel = OBJ_NEW('IDLitVisContourLevel', NAME='Level 1')
    self._oLevels->Add, oLevel, /AGGREGATE, /NO_UPDATE
    ...
    if (ncontained lt nlevels) then begin
        ...
        for i=ncontained, nlevels-1 do begin
            ...
            for p=0,N_ELEMENTS(props)-1 do begin
                ...
                if (~props[p] || props[p] eq 'MY_VALUE') then $
                    continue
        ...
        self._oLevels->Add, oLevels[ncontained:*], /AGGREGATE, /NO_UPDATE
...
pro IDLitVisContour::GetProperty, $
    ...
    if (N_ELEMENTS(_extra) gt 0) then begin
        ...
        ; See Note in ::Init.
        levelProps = WHERE(STRCMP(_extra, 'MY_', 3) or $
            STRCMP(_extra, 'FONT_', 5) or $
            (_extra eq 'TICKFORMAT') or $
            (_extra eq 'TICK_DEFINEDFORMAT'), nProps, $
...
pro IDLitVisContour::SetProperty, $
    ...
    if keyword_set(n_levels) then begin
        ...
        IF (n_levels GT n_elements(c_valueTemp)) THEN BEGIN
          self->_FillParameter,'C_COLOR',cColorOld,TWOD=~self._palColor
...
pro IDLitVisContour::OnDataChangeUpdate, oSubject, parmName
    ...
    case STRUPCASE(parmName) of
        ...
        'Z': BEGIN
          ...
          self->IDLitVisContour::GetProperty, MY_LABELLING=labelling

          ; Force a recalculation of the tick interval/length and the labels.
          self->IDLitVisContour::SetProperty, $
            MY_LABELLING=labelling, $
            TICKINTERVAL=self._tickinterval, TICKLEN=self._ticklen
         ...
         'PALETTE': begin
            if(oSubject->GetData(data)) then begin
                ...
                if OBJ_VALID(oLevels[0]) then begin
                    for i=0, nLevels-1 do begin
                        oLevels[i]->SetPropertyAttribute, SENSITIVE=0, 'MY_COLOR'
                    endfor
                endif
            end
         end
