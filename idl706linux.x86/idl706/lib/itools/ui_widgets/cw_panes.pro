; $Id: //depot/idl/IDL_70/idldir/lib/itools/ui_widgets/cw_panes.pro#2 $
;
...
function CW_PANES, parent, LEFT_CREATE_PRO=left_createpro, $
  ...
  if allowCollapse then begin
    ...
    state.arrow1 = widget_button(state.line_base,xsize=8, $
                               ysize=11,x_bitmap_extra=4, $
                               value=transpose(byte([8,12,14,15,14,12,8])), $
                               event_PRO='CW_PANES_LEFT_BUTTON')
    ;;create right arrow button
    state.arrow2 = widget_button(state.line_base,xsize=8, $
                               ysize=11,x_bitmap_extra=4, $
                               value=transpose(byte([1,3,7,15,7,3,1])), $
