; $Id: //depot/idl/IDL_70/idldir/lib/itools/ui_widgets/idlitwdimportwizard.pro#2 $
;---------------------------------------------------------------------------
...
function _idlitWdIW_FileSel, id, pState
    ...
    if(bThere)then $
      wButton = widget_button(wTmp, value=filename, /bitmap, uname='FILE_SEL') $
    else $
...
function _idlitWdIW_CommandLine, id, pState
    ...
    wTmp = Widget_label(wBase, $
                        ...
                        /align_left)
    wBImport = Widget_Base(wBase,/row,xpad=10,space=5)
    wDataType = Widget_DropList(wBImport, /dynamic, uname="DATATYPE")
