; $Id: //depot/idl/IDL_70/idldir/lib/itools/ui_widgets/idlitwdprintpreview.pro#2 $
;
...
PRO IDLitwdPrintPreview_RecalcGeom, pState
  ...
  if(landscape)then begin
    xSize = fix(szScreen[0]*.7)
    ySize = fix(xSize*aspect)
  endif else begin
    ySize = fix(szScreen[1]*.7)
...
function IDLitwdPrintPreview, oUI, oTarget
  ...
  s_pgBoarder  = 75   ;; space around the edge
  ...
  wButtons[1] = widget_button(wRowBaseLower, $
                              ...
                              uname="HELP")
  geo = widget_info(wButtons[1], /geometry)
  wLabel = widget_label(wRowBaseLower, $
                        value=IDLitLangCatQuery('UI:wdPrintPreview:Orientation'))
  wDList = widget_droplist(wRowBaseLower, $
  ...
  wBBase = widget_base(wRowBaseLower, /row, /nonexclusive, ysize=geo.scr_ysize, $
                       /align_center)
  ...
  wButtons[3] = widget_button(wRowBaseLower, $
                              ...
                              uname="PRINT")
  wButtons[4] = widget_button(wRowBaseLower, $
  ...
  IF (!version.os_family EQ 'unix') THEN $
    widget_control, wButtons[2], sensitive=0
