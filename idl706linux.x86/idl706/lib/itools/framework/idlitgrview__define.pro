; $Id: //depot/idl/IDL_70/idldir/lib/itools/framework/idlitgrview__define.pro#2 $
;
...
pro IDLitgrView::_RegisterProperties, $
    ...
    if (registerAll || (updateFromVersion lt 610)) then begin
        ...
        self->RegisterProperty, 'MARGIN_2D_X', $
            ...
            /FLOAT, VALID_RANGE=[0.0d,0.49d,0.01d]
...
pro IDLitgrView::GetProperty, $
    ...
    if (ARG_PRESENT(margin2DX) || ARG_PRESENT(margin2DY)) then begin
...
pro IDLitgrView::SetProperty, $
    ...
    ; MARGIN_2D_X and MARGIN_2D_Y simply get passed along to layers.
    if ((N_ELEMENTS(margin2DX) gt 0) || $
        (N_ELEMENTS(margin2DY) gt 0)) then begin
        oLayers = self->Get(ISA='IDLitgrLayer', /ALL, COUNT=nLayers)
        for i=0,nLayers-1 do begin
            if (~OBJ_ISA(oLayers[i], 'IDLitgrAnnotateLayer')) then begin
    ...
    if (bReScroll) then begin
        ...
        if (nLayers gt 0) then begin
            ...
            for i=0,nLayers-1 do begin
                oViewLayerArr[i]->_UpdateMargins, origVirtualDims, zoomOnResize
