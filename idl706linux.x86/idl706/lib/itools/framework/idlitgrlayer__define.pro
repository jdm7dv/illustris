; $Id: //depot/idl/IDL_70/idldir/lib/itools/framework/idlitgrlayer__define.pro#2 $
;
...
pro IDLitgrLayer::_RegisterProperties, $
    ...
    if (registerAll || (updateFromVersion lt 610)) then begin
        ; MARGIN_2D_X and MARGIN_2D_Y are registered, but hidden.
        ; These properties are exposed to the user via the parent view.
        self->RegisterProperty, 'MARGIN_2D_X', $
            NAME='X margin', $
            DESCRIPTION='Normalized width of margin', $
            /FLOAT, /HIDE, VALID_RANGE=[0.0d,0.49d,0.01d]
        self->RegisterProperty, 'MARGIN_2D_Y', $
            NAME='Y margin', $
...
pro IDLitgrLayer::GetProperty, $
    ...
    MARGIN_2D_X=margin2DX, $
    MARGIN_2D_Y=margin2DY, $
    STRETCH_TO_FIT=stretchToFit, $
...
pro IDLitgrLayer::SetProperty, $
    ...
    if (bWinDimChange) then begin
        ...
        if (bRecomp2DPadding) then begin
            ...
            origVirtualViewDims = oView->GetVirtualViewport(oWin, /UNZOOMED)
            self->_UpdateMargins,  origVirtualViewDims, zoomOnResize, $
...
function IDLitgrLayer::_RoundMargin, inMargin
    ...
    outMargin = (ROUND(inMargin*100.0d)/100.0d) > 0.0
...
;
pro IDLitgrLayer::_UpdateMargins, virtualViewDims, zoomOnResize, $
