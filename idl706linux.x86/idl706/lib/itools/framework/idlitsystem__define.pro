; $Id: //depot/idl/IDL_70/idldir/lib/itools/framework/idlitsystem__define.pro#2 $
;
...
@idlit_catch.pro
   ...
   toolName = KEYWORD_SET(title) ? title : "IDL iTool"
   ...
   if (OBJ_VALID(oUIDesc)) then begin
    ...
    if (N_ELEMENTS(viewGrid) gt 0) then begin
        oWin = oTool->GetCurrentWindow()
        if (OBJ_VALID(oWin)) then $
                oWin->SetProperty, VIEW_GRID=viewGrid
    endif
...
PRO IDLitSystem::_SetCurrentTool, oTool

    compile_opt idl2, hidden

    ; Assume all error checking on oTool has been done by caller.

    if (self._oCurrentTool eq oTool) then $
        return
...
;;                exist, be valid and part of the system.
;;
PRO IDLitSystem::SetCurrentTool, idTool
   ...
   self->IDLitSystem::_SetCurrentTool, oTool
