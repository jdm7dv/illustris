; $Id: //depot/idl/IDL_70/idldir/lib/itools/framework/_idlitmapprojection__define.pro#2 $
;
...
@map_proj_init_commonblock
    ...
    self->RegisterProperty, 'IDATUM', $
        NAME='Ellipsoid (datum)', $
...
@map_proj_init_commonblock
    ...
    ; Use either my own datum or a predefined one.
    if (self._datum eq 'User defined') then begin
...
@map_proj_init_commonblock
    ...
    ; Are we using NAD27 or NAD83 datum?
    isNAD27 = self._datum eq 'Clarke 1866'
    zones = isNAD27 ? $
...
;----------------------------------------------------------------------------
; If datum changes, update the semimajor & semiminor values.
;
pro _IDLitMapProjection::_UpdateDatum

    compile_opt idl2, hidden

@map_proj_init_commonblock

    userDefined = self._datum eq 'User defined'
    ...
    index = (WHERE(c_EllipsoidNames eq self._datum))[0]
    if (index lt 0) then $
        return
    ...
    if (self._projection eq 'State Plane' && self._datum eq 'Clarke 1866' && $
...
@map_proj_init_commonblock
    ...
    'UTM': begin
        ...
        self->SetPropertyAttribute, 'IDATUM', ENUMLIST=c_EllipsoidNames
        break
        end
    ...
    'State Plane': begin
        ...
        ; Restrict the allowed datums.
        self->SetPropertyAttribute, 'IDATUM', ENUMLIST=[nad27, nad83]
    ...
    self->SetPropertyAttribute, 'IDATUM', $
        SENSITIVE=(self._projection ne 'No projection')

    ; If not state plane, put our datums back.
...
pro _IDLitMapProjection::GetProperty, $
    ...
    DATUM=strDatum, $  ; command-line support
    IDATUM=idatum, $
...
@map_proj_init_commonblock
    ...
    if (ARG_PRESENT(strDatum)) then $
        strDatum = self._datum

    if (ARG_PRESENT(idatum)) then begin
        ; Convert from our string back to an index.
        self->GetPropertyAttribute, 'IDATUM', ENUMLIST=datums
        idatum = (WHERE(STRCMP(datums, self._datum, /FOLD_CASE)))[0] > 0
    endif
    ...
    if (ARG_PRESENT(gctpzone)) then begin
        ...
        'State Plane': begin
            if (self._datum eq 'Clarke 1866') then begin
...
@map_proj_init_commonblock
    ...
    if (N_ELEMENTS(mapProjection) eq 1) then begin
        ...
        if (match ge 0) then begin
            ...
            ; (such as available datums) to be changed.
            self->_UpdateProjection
        endif else begin
            self->ErrorMessage, SEVERITY=1, $
                IDLitLangCatQuery('Warning:MapProjection:UnknownProjection') + $
    ...
    ; command-line support
    if (N_ELEMENTS(strDatum) eq 1) then begin
        ; DATUM keyword should only come in from the command line,
        ; and should be a string.
        ; Convert from enumlist index to a string.
        self->GetPropertyAttribute, 'IDATUM', ENUMLIST=datums
        ; Look for a match of the starting n characters regardless of
        ; extra spaces or case, where n is the length of the string.
        sDatum = STRCOMPRESS(strDatum, /REMOVE)
        match = (WHERE(STRCMP(STRCOMPRESS(datums, /REMOVE), $
            sDatum, STRLEN(sDatum), /FOLD_CASE)))[0]
        ; Set property using the datum index.
        if (match ge 0) then begin
            self->_IDLitMapProjection::SetProperty, IDATUM=match
        endif else begin
            self->ErrorMessage, SEVERITY=1, $
    ...
    if (N_ELEMENTS(idatum) eq 1) then begin
        ; Convert from an index into our string.
        self->GetPropertyAttribute, 'IDATUM', ENUMLIST=datums
        self._datum = datums[idatum < (N_ELEMENTS(datums)-1)]
    ...
    if (N_ELEMENTS(semiMajor) eq 1 && self._semiMajor ne semiMajor) then begin
        ...
        self->_IDLitMapProjection::GetProperty, IDATUM=idatum
        if (idatum ne 0) then $
            self->_IDLitMapProjection::SetProperty, IDATUM=0
    endif
    ...
    if (N_ELEMENTS(statePlaneZone27) eq 1) then begin
        ...
        self->_UpdateStatePlane
        self->_UpdateDatum   ; needed for Michigan ellipsoid
    endif
