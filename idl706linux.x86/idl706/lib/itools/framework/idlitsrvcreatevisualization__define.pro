; $Id: //depot/idl/IDL_70/idldir/lib/itools/framework/idlitsrvcreatevisualization__define.pro#2 $
;
...
function IDLitsrvCreateVisualization::UndoOperation, oCommandSet, $
  ...
  oCmds = oCommandSet->Get(/all, count=nObjs)
    ...
    if (OBJ_ISA(oView,'IDLitgrView')) then begin
        ...
        if (~iStatus) then $
            oView = OBJ_NEW()
        iStatus = oCmds[0]->GetItem("ORIG_MARGIN_2D_X", margin2Dx)
        if (~iStatus) then $
            oView = OBJ_NEW()
        iStatus = oCmds[0]->GetItem("ORIG_MARGIN_2D_Y", margin2Dy)
        if (~iStatus) then $
            oView = OBJ_NEW()
...
function IDLitsrvCreateVisualization::_Create, oVisDesc, oData, $
   ...
   if (OBJ_VALID(oView) && transact) then begin
      ...
      if (~iStatus) then $
          OBJ_DESTROY, oCmd
      iStatus = oCmd->AddItem("ORIG_MARGIN_2D_X", margin2Dx)
      if (~iStatus) then $
          OBJ_DESTROY, oCmd
      iStatus = oCmd->AddItem("ORIG_MARGIN_2D_Y", margin2Dy)
      if (~iStatus) then $
          OBJ_DESTROY, oCmd
