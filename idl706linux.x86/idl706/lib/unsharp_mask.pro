; $Id: //depot/idl/IDL_70/idldir/lib/unsharp_mask.pro#2 $
...
function unsharp_mask, array, $
    ...
    if (ndim eq 3) then begin   ; multichannel

        smoothed = MAKE_ARRAY(dims, /NOZERO, TYPE=type)

        for ch=0,dims[true - 1]-1 do begin
            case true of
                1: smoothed[ch, *, *] = $
                    CONVOL(CONVOL(REFORM(array[ch, *, *]), $
                    gaussian, norm, /EDGE_WRAP), $
                    TRANSPOSE(gaussian), norm, /EDGE_WRAP)
                2: smoothed[*, ch, *] = $
                    CONVOL(CONVOL(REFORM(array[*, ch, *]), $
                    gaussian, norm, /EDGE_WRAP), $
                    TRANSPOSE(gaussian), norm, /EDGE_WRAP)
                3: smoothed[0, 0, ch] = CONVOL(CONVOL(array[*, *, ch], $
                    gaussian, norm, /EDGE_WRAP), $
                    TRANSPOSE(gaussian), norm, /EDGE_WRAP)
            endcase
        endfor
