; $Id: //depot/idl/IDL_70/idldir/lib/parse_url.pro#2 $
;
; Copyright (c) 2006-2008, ITT Visual Information Solutions. All
...
function Parse_Url, url
  ...
  if (iPosAt ne -1) then begin
     ...
     if ((iPosPass ne -1) && (iPosPass lt iPosAt)) then begin
         ...
         ; store the password component in the retrun structure
         xUrlProps.Password = strmid(url, iPosPass+1, iLen)
     endif else begin
         iPosPass = iPosAt
     endelse
     iLen = iPosPass - iPos
     ; store the username component in the retrun structure
  ...
  if (iPosPort ne -1) then begin
     ...
     ; store the port component in the retrun structure
     xUrlProps.Port = strmid(url, iPosPort+1, iLen)
     iHostEnd = iPosPath - iLen -1
  endif

  ; store the host component in the retrun structure
  iLen = iHostEnd - iPos
  ...
  ; store the path component in the retrun structure
  iLen = iPosQuery - (iPosPath + 1)
  xUrlProps.Path = strmid(url, iPosPath+1, iLen)

  ; store the query component in the retrun structure
