; $Id: //depot/idl/IDL_70/idldir/lib/map_proj_init.pro#2 $
;
...
;---------------------------------------------------------------------------
; Check datum code and convert to semimajor/minor axes.
; If Datum is a string, replaces its value with the corresponding index.
;
function map_proj_getdatum, datum
...
@map_proj_init_commonblock
    ...
    ; String was supplied?
    if (SIZE(datum, /TYPE) eq 7) then begin
        ; Find closest matching datum name.
        datumIn = datum
        datumCheck = STRCOMPRESS(datumIn, /REMOVE_ALL)
        allDatums = STRCOMPRESS(c_EllipsoidNames, /REMOVE_ALL)
        datum = (WHERE(STRCMP(allDatums, datumCheck, $
            STRLEN(datumCheck), /FOLD_CASE), nMatch))[0]
        case nMatch of
            0: MESSAGE, /NONAME, $
                'Invalid value for keyword DATUM: ' + datumIn
            1: ; Valid datum.
            else: MESSAGE, /NONAME, 'Ambiguous DATUM abbreviation: ' $
                + datumIn
        endcase
    endif else begin
        ; Make sure datum number is within range.
        datum = LONG(datum)
        if (datum ge N_ELEMENTS(c_EllipsoidNames)) then $
            MESSAGE, /NONAME, $
            'Invalid value for keyword DATUM: ' + STRTRIM(datum)
    endelse

    result = [c_EllipsoidMajor[datum], c_EllipsoidMinor[datum]]
...
@map_proj_init_commonblock
    ...
    datum = (N_ELEMENTS(datumIn) gt 0) ? datumIn : $
        (c_ProjParameters[0,index] eq 'SPHERE_RADIUS') ? 19 : 0
    projParams[0:1] = MAP_PROJ_GETDATUM(datum)
    ...
    ; If necessary,set datum to -1 to alert GCTP that the user
    ; supplied the semimajor axis or radius, rather than a datum code.
    ; This is irrelevant for IDL projections.
    if (TOTAL((suppliedKeywords eq 'SEMIMAJOR_AXIS') or $
        (suppliedKeywords eq 'SPHERE_RADIUS')) gt 0) then datum = -1
...
function map_proj_init, projectionIn, $
    DATUM=datumIn, $     ; Spheroid or Datum code.
    ...
    if (sMap.projection eq 20) then begin   ; GCTP projection
        ...
        MAP_PROJ_GCTP_FORINIT, sMap.simple, gctpZone, projParams, datum
        MAP_PROJ_GCTP_REVINIT, sMap.simple, gctpZone, projParams, datum
