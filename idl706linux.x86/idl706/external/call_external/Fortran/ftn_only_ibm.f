C
C	$Id: //depot/idl/IDL_70/idldir/external/call_external/Fortran/unix/ftn_only_ibm.f#1 $
C
C NAME:
C 	ftn_only
C
...
C
C RESTRICTIONS:
C	This example is setup to run using the IBM operating system.
C
C EXAMPLE:
C-----------------------------------------------------------------------------
...
C$Function FTN_ONLY
        ...
        REAL*8 FUNCTION FTN_ONLY(ARGC, ARGV)
...
C       Fortran from IDL.
 ...
 140    FORMAT(10X,'Double Parameter:',T50,F4.1)
 	...
 	WRITE(*,145)
 145	FORMAT(10X,'Float Array:')

	WRITE(*,150)(I, FLOATARR(I), I=1, N)
 150	FORMAT(15X,'Element ',I3,', Value: ',T47, F7.2)
