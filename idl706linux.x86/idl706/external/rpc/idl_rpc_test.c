 ...
 *  rpctest.c
 *
 *  $Id: //depot/idl/IDL_70/idldir/external/rpc/idl_rpc_test.c#2 $
 *  
 *
 *
 ...
 */
   ...
   type_s.ui = 22;
   IDL_RPCStoreScalar(vPtr, IDL_TYP_UINT, &type_s);
   if(!IDL_RPCSetMainVariable(client, "V_UINT", vPtr)){
      fprintf(stderr, "error setting variable in server: unsigned int");
      IDL_RPCDeltmp(vPtr);
      return 1;
   }
   vTmp = IDL_RPCGetMainVariable(client, "V_UINT");
   if(vTmp->type != IDL_TYP_UINT || vTmp->value.ui != 22){
      fprintf(stderr, "IDL_RPCGetMainVariable: Error getting variable\n");
       return 1;
   }
   IDL_RPCDeltmp(vTmp);
   ...
   type_s.ul = 33;
   IDL_RPCStoreScalar(vPtr, IDL_TYP_ULONG, &type_s);
   if(!IDL_RPCSetMainVariable(client, "V_ULONG", vPtr)){
      fprintf(stderr, "error setting variable in server: unsigned long");
      IDL_RPCDeltmp(vPtr);
      return 1;
   }
   vTmp = IDL_RPCGetMainVariable(client, "V_ULONG");
   if(vTmp->type != IDL_TYP_ULONG || vTmp->value.ul != 33){
      fprintf(stderr, "IDL_RPCGetMainVariable: Error getting variable\n");
       return 1;
   }
   IDL_RPCDeltmp(vTmp);
   ...
   type_s.l64 = 44;
   IDL_RPCStoreScalar(vPtr, IDL_TYP_LONG64, &type_s);
   if(!IDL_RPCSetMainVariable(client, "V_LONG64", vPtr)){
      fprintf(stderr, "error setting variable in server: long 64");
      IDL_RPCDeltmp(vPtr);
      return 1;
   }
   vTmp = IDL_RPCGetMainVariable(client, "V_LONG64");
   if(vTmp->type != IDL_TYP_LONG64 || vTmp->value.l64 != 44){
      fprintf(stderr, "IDL_RPCGetMainVariable: Error getting variable\n");
       return 1;
   }
   IDL_RPCDeltmp(vTmp);
   ...
   type_s.ul64 = 55;
   IDL_RPCStoreScalar(vPtr, IDL_TYP_ULONG64, &type_s);
   if(!IDL_RPCSetMainVariable(client, "V_ULONG64", vPtr)){
      fprintf(stderr, "error setting variable in server: unsigned long64");
      IDL_RPCDeltmp(vPtr);
      return 1;
   }
   vTmp = IDL_RPCGetMainVariable(client, "V_ULONG64");
   if(vTmp->type != IDL_TYP_ULONG64 || vTmp->value.ul64 != 55){
      fprintf(stderr, "IDL_RPCGetMainVariable: Error getting variable bb\n");
       return 1;
   }
   IDL_RPCDeltmp(vTmp);
   ...
   if(IDL_RPCExecuteStr(client, "HELP") != 1){
...
int RPCGetArrayTest(CLIENT *pClient)
{
  ...
  if(IDL_RPCExecuteStr(pClient, "VA_BYTE=bindgen(15)") != 1){
   ...
   if(IDL_RPCExecuteStr(pClient, "VA_INT=indgen(31)") != 1){
      fprintf(stderr,"Error executing command string\n");
      return 1;
   }
   ...
   if(IDL_RPCExecuteStr(pClient, "VA_LONG=lindgen(30)") != 1){
      fprintf(stderr,"Error executing command string\n");
      return 1;
   }
   ...
   if(IDL_RPCExecuteStr(pClient, "VA_LONG64=l64indgen(30)") != 1){
      fprintf(stderr,"Error executing command string\n");
      return 1;
   }
   pVtmp = IDL_RPCGetMainVariable(pClient, "VA_LONG64");
   if(IDL_RPCGetVarType(pVtmp) != IDL_TYP_LONG64 || !IDL_RPCVarIsArray(pVtmp) ||
      IDL_RPCGetArrayNumElts(pVtmp) != 30){
      fprintf(stderr, "IDL_RPCGetMainVariable: Error getting long64 array\n");
      return 1;
   }
   IDL_RPCDeltmp(pVtmp);

   if(IDL_RPCExecuteStr(pClient, "VA_ULONG64=ul64indgen(30)") != 1){
      fprintf(stderr,"Error executing command string\n");
      return 1;
   }
   pVtmp = IDL_RPCGetMainVariable(pClient, "VA_ULONG64");
   if(IDL_RPCGetVarType(pVtmp) != IDL_TYP_ULONG64 || !IDL_RPCVarIsArray(pVtmp) ||
      IDL_RPCGetArrayNumElts(pVtmp) != 30){
      fprintf(stderr, "IDL_RPCGetMainVariable: Error getting unsigned long64 array\n");
      return 1;
   }
   IDL_RPCDeltmp(pVtmp);


   if(IDL_RPCExecuteStr(pClient, "VA_FLOAT=findgen(31)") != 1){
      fprintf(stderr,"Error executing command string\n");
      return 1;
   }
   ...
   if(IDL_RPCExecuteStr(pClient, "VA_DOUBLE=dindgen(30)") != 1){
      fprintf(stderr,"Error executing command string\n");
      return 1;
   }
   ...
   if(IDL_RPCExecuteStr(pClient, "VA_COMPLEX=cindgen(21)") != 1){
      fprintf(stderr,"Error executing command string\n");
      return 1;
   }
   ...
   if(IDL_RPCExecuteStr(pClient, "VA_DCOMPLEX=dcindgen(7)") != 1){
      fprintf(stderr,"Error executing command string\n");
      return 1;
   }
   ...
   if(IDL_RPCExecuteStr(pClient, "VA_STRING=sindgen(32)") != 1){
      fprintf(stderr,"Error executing command string\n");
      return 1;
   }
   ...
   IDL_RPCDeltmp(pVtmp);
   if(IDL_RPCExecuteStr(pClient, "HELP") != 1){
...
int RPCSetArrayTest(CLIENT *pClient)
{
   ...
   IDL_MEMINT   l_dim1 = 100;
   IDL_MEMINT   l_dim2[2] = {10,9}; 
   int          i,j;
 ...
 * Now a unsigned int
 */
   pUInt = (IDL_UINT*)IDL_RPCMakeArray(IDL_TYP_UINT, 1, &l_dim1, 
				    IDL_BARR_INI_ZERO, &pVptr);
   if(!IDL_RPCSetMainVariable(pClient, "SET_UINT", pVptr)){
     fprintf(stderr,"error setting output variable\n");
     return 1;
   }
   IDL_RPCDeltmp(pVptr);
   pVptr = IDL_RPCGetMainVariable(pClient, "SET_UINT");
   if(!pVptr){
      fprintf(stderr, "IDL_RPCGetVariable: Error getting variable SET_UINT.\n");
      return 1;
    }
/*
 * Does result look sane?
 */
   if(IDL_RPCGetVarType(pVptr) != IDL_TYP_UINT || !IDL_RPCVarIsArray(pVptr) ||
      IDL_RPCGetArrayNumElts(pVptr) != l_dim1){
      fprintf(stderr, "Return unsigned int variable attributes wrong.\n");
      return 1;
   }
   IDL_RPCDeltmp(pVptr);
 ...
 */
   ...
   if(IDL_RPCExecuteStr(pClient, "HELP, SET_BYTE, SET_INT, SET_LONG, set_float") != 1){
      fprintf(stderr, "IDL_RPCExecuteStr: Error executing help command\n");
      return 1;
   }
   return 0; 
 ...
 */
   if(IDL_RPCExecuteStr(pClient, "mem0=(memory())(0)") != 1){
